...
class Google_REST {
  ...
  static public function execute(Google_HttpRequest $req) {
    ...
    $decodedResponse = self::decodeHttpResponse($httpRequest);
      ...
      $fp = fopen(Yii::getPathOfAlias('application.runtime') . "/google_request.txt", 'a+');

      fwrite($fp,
          "\n\n----------- REQUEST: " . date('Y-m-d H:i:s') . "-------------------\n" .
          print_r($req->getPostBody(),1).
          "\n=====\n".
          print_r( json_decode( (string)$req->getPostBody(), true) , 1) .
          "\n----------- EOF -------------------\n\n"
      );
      fwrite($fp,
          "\n----------- RESPONSE: " . date('Y-m-d H:i:s') . "-------------------\n" .
          print_r($decodedResponse,1).
          "\n----------- EOF -------------------\n\n"
      );
      fclose($fp);

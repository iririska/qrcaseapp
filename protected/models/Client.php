...
class Client extends CActiveRecord
{
    ...
    {
        //DOB
        if (!empty($this->dob) && Controller::validateDate($this->dob)) $this->dob = date('Y-m-d', strtotime($this->dob));
    ...
    public static function getMyClients(){
        ...
        if (!Yii::app()->user->getIsAdmin()) {
            $criteria->with = array('client_user');
            $criteria->addCondition('client_user.user_id = :id');
        ...
        foreach (Client::model()->findAll($criteria) as $_client) {
            /* @var $_client Client * /
            $clients[] = array(
                'id'=>$_client->id,
                'label'=>"$_client->firstname $_client->lastname ($_client->email)",
            );
        }*/

        return CHtml::listData( Client::model()->findAll(), 'id' , 'fullname' );
